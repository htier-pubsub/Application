...
public class StreamBridge {
    ...
    public JsonNode sendCryptoOperation(String operation, String data, Integer length) {
        try {
        ...
        try {
            while (true) {
                ...
                if (modbus.hasStateChanged()) {
                    ...
                    JsonNode cryptoResult = bridge.sendCryptoOperation("random_hex", null, 16);
                    if (cryptoResult != null && cryptoResult.has("success") && 
                        cryptoResult.get("success").asBoolean()) {
                        String hexValue = cryptoResult.get("data").get("result").asText();
                        System.out.println("✓ Generated hex: " + hexValue);
                    } else {
                        System.out.println("✗ Failed to generate hex");
                    }
                    
                    JsonNode hashResult = bridge.sendCryptoOperation("sha256", timestamp, null);
                    if (hashResult != null && hashResult.has("success") && 
                        hashResult.get("success").asBoolean()) {
                        String hashValue = hashResult.get("data").get("result").asText();
                        System.out.println("✓ SHA256 of timestamp: " + hashValue.substring(0, 16) + "...");
                    } else {
